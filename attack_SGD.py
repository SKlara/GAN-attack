...
class Discriminator():
    def __init__(self):
        ...
        optimizer = SGD(lr=1e-3, momentum=0.0, decay=1e-7)
...
class CGAN():
    def __init__(self):
        ...
        optimizer = SGD(lr=0.02, momentum=0.0, decay=1e-7)
    ...
    def sample_images(self, n, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("attack_images/images_22/%d_%d.png" % (n, epoch))
        plt.close()
...
class Adversary():
    ...
    def train(self, epochs, n):
        self.cgan.train((epochs*3), self.x_zero, self.y_zero, self.x_one, self.y_one, n)
        self.discriminator.train(epochs, self.x_zero, self.y_zero, self.x_one, self.y_one)
